 ...
 * 2015-4-24 daidd add 增加菜单回调函数传入Event对象
 * 2015-7-21 江毅   修复基于位置服务需要准确获取到原始右击位置所在的坐标信息。
 */
	...
	$.fn.smartMenu = function(data, options) {
			...
			if (!jqueryMenu.size()) {
				...
				$(idKey + params.name +" a").bind("click", function(e) {
					...
					if ($.isFunction(callback)) {
						e=rightclickEvent||e;//用原始右击的坐标事件替换
		...
		$(this).each(function() {
			this.oncontextmenu = function(e) {
				...
				e = e ||  window.event;
				rightclickEvent=$.extend(true,{},e);//保存右击事件信息//通过克隆对象解决ie8下的用window.event是的信息改变问题
				//阻止冒泡
				e.cancelBubble = true;
