...
#daidd modify 2015-05-08
#江毅  modify 2015-08-12 支持选中标签不默认居中问题
 增加点击时间时的回调函数
---------------------------------- */
...
jQuery.fn.timelinr = function(options){
	...
	settings = jQuery.extend({
		...
		onDateSelected:             null,               //增加选择时间的回调函数
		centeredSelectedItem:		true				//选中的标签是否居中
	}, options);
	...
	$(function(){
		...
		var lastSeeIndex = 999;
		var firstSeeIndex = -999;
		
		// set positions!
		
		if(settings.centeredSelectedItem){
			if(settings.orientation == 'horizontal') {	
				$(settings.issuesDiv).width(widthIssue*howManyIssues);
				$(settings.datesDiv).width(widthDate*howManyDates).css('marginLeft',"8px");
			...
			var timeLineBg = $(settings.containerBgDiv).width();
			lastSeeIndex = parseInt((timeLineBg-10)/widthDate) - 2;
			...
			if(settings.orientation == 'horizontal') {	
				...
				$(settings.issuesDiv).height(heightIssue*howManyIssues);
				$(settings.datesDiv).height(heightDate*howManyDates).css('marginTop',heightDate);
				var defaultPositionDates = parseInt($(settings.datesDiv).css('marginTop').substring(0,$(settings.datesDiv).css('marginTop').indexOf('px')));
			}
		}
		...
		$(settings.datesDiv+' a').click(function(event){
			...
			if(settings.centeredSelectedItem){
				...
				var addIndex = -1;
				var needMove = false;
				if(currentIndex+1 < howManyDates){
					addIndex = currentIndex - lastSeeIndex;
				}
				...
				if(currentIndex > 0 && currentIndex == firstSeeIndex ){
					addIndex = firstSeeIndex - 1;
					needMove = true;
				}
					
				if( needMove && settings.orientation == 'horizontal' ) {
					$(settings.datesDiv).animate({'marginLeft':defaultPositionDates-(widthDate*addIndex)},{queue:false, duration:'settings.datesSpeed'});
					firstSeeIndex = addIndex;
				} else if(needMove && settings.orientation == 'vertical') {
					$(settings.datesDiv).animate({'marginTop':defaultPositionDates-(heightDate*currentIndex)},{queue:false, duration:'settings.datesSpeed'});
				}
			}
			
			if(settings.onDateSelected){
				//var onDateSelectedCallback = settings.onDateSelected;
				settings.onDateSelected.call(this,arguments[0]);
			}
...
function stopPlay(){
	clearInterval(settings.currentInterval);
}

function startPlay(){

	settings.currentInterval =  setInterval("autoPlay()", settings.autoPlayPause);
}