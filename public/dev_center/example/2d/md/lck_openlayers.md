...
?> 矢量瓦片Web端openlayers SDK是基于openlayers 1.0以上扩展开发而来，本章节主要对矢量瓦片web端SDK接口进行相关说明。
...
| 名称            | 描述                          | 继承自               |
| --------------- | ----------------------------- | ---------------------|
|OpenLayers.Layer.GXYZ                 | 后端绘制矢量瓦片底图图层     | OpenLayers.Layer.XYZ |
|OpenLayers.Layer.GWVTAnno             | 前端注记避让前端绘制注记图层 | OpenLayers.Layer     |



## OpenLayers.Layer.GXYZ

描述：后端绘制矢量瓦片底图图层 。OpenLayers的OpenLayers.Layer.XYZ, 它的扩展方法如下：

| 名称            | 参数                          | 返回值               | 含义                |
| --------------- | ----------------------------- | ---------------------|---------------------|
|initialize(name, url, options)           |String,String,Json                                           | 无               |构造函数,name为图层名        |
|setFilter(filter)                       | Custom.FilterLayer                                           | 无              | 给底图数据设置过滤条件     |
|getFeatureByXY(x,y,callback)            |int,int,function                                              | JSON             |根据屏幕坐标拾取要素        |
|getFeatureByLonlat(latLng,callback)     |OpenLayers.LonLat,function                                             | JSON             |根据地理坐标拾取要素        |
...
##	OpenLayers.Layer.GWVTAnno
描述：前端注记避让前端绘制注记图层。继承自OpenLayers的OpenLayers.Layer图层，它的扩展方法如下：
...
|addToMap(map)          | OpenLayers.Map  | 无                   | 将注记图层加入到Map中 |
...
## openlayers2 官方API
[http://dev.openlayers.org/docs/files/OpenLayers-js.html](http://dev.openlayers.org/docs/files/OpenLayers-js.html)