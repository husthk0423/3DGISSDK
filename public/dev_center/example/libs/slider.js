; +
    (function () {
            ...
            createSlider(sliderList) {
                ...
                sliderList.forEach(item => {
                    this.silder({
                        ...
                        disable: false,
                        callBackMove: (val) => {
                            (item.callBackMove && typeof item.callBackMove == 'function') && item.callBackMove(val);
                        },
                        callBackMouseup: (val) => {
                            (item.callBackMouseup && typeof item.callBackMouseup == 'function') && item.callBackMouseup(val);
                        }
                    });
                });
            }
        ...
        function Silder(opt) {
            ...
            ss.max = silder_defult.max;
            ss.step = silder_defult.step;
        ...
        Silder.prototype.createTemplate = function () {
            ...
            <div class="zui-slider ${ss.disable ? 'zui-disabled' : ''}">
                <div class="zui-slider-tips" style="left: ${ss.pos}; display: none">${ss.pos.split('%')[0] || 0}</div>
        ...
        `;
            ...
            var ss = e.data.that;
            ss.num = Math.round(((e.clientX - e.data.silderLeft) / e.data.silderWidth) * 100);
            if (ss.num > 100 || ss.num < 0) {
                return;
            }
            const num = ((ss.num) / 100 * ss.max).toFixed(1);
            ss.callBackMove(num);
            ss.silderBar.css('width', Math.round(ss.num) + '%');
            ss.silderWrap.css('left', Math.round(ss.num) + '%');
            ss.silderTips.css('left', Math.round(ss.num) + '%');
            ss.silderTips.text(num);
        };
    })(window);